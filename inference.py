...
def predict_submission(
    ...
    if task != Task.All:
        ...
        for t in TASKS: # 불러온 파라미터 파일명에 dependent하므로 파일명 형식 유지 TODO: json 파일로 메타정보를 저장해두면 좋을 것 같다
            ...
            pred_df.to_csv(
                os.path.join(save_path, submission_dir, f"submission_{t}.csv"), index=False
            )
        ...
        submission['ans'] = submission.apply(lambda x: label_enc.transform(tuple(x.iloc[1:]), task='main'), axis=1) # 미래의 지형아 미안하다
